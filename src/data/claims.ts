...
export const CLAIMS: ReconstructionClaim[] = [
  ...
  },
  {
    ...
    sourceIds: ['esv-bible'],
    notes:
      'Governed by ADR-013 (docs/architecture-decisions/adr-013-narrated-supernatural-depiction.md): the communication event itself — "the word of the LORD came to Nathan" — is stated by card only, never visualized, at the b-night-word beat or anywhere else in this scene; see claim-oracle-depiction for the full rendering-policy disclosure and why this case is the purest ADR-013 test to date (queue #28). Consolidates 7:17 (Nathan’s report) into this claim per the brief’s explicit allowance and the gibeon-pool claim-consolidation precedent, since the report stages no content beyond what this claim already carries. The card’s own narration of 7:4-15 is written in the text’s first-person divine idiom ("I took you...", "I will make you...") as the project’s own paraphrase, not a verbatim ESV quote — only 7:16’s closing line is a live excerpt, spending part of 2sam-7’s ESV budget (see the 2sam-7 passage entry). No figure, name, or visual cue anywhere identifies Solomon or any specific son as the "offspring" of 7:12 — the text does not name him here and neither does this claim or the scene. Redaction-critical reading, disclosed per queue #29: 2 Samuel 7:13a ("he shall build a house for my name") is widely read by critical scholarship as a later theological expansion anticipating Solomon and the temple, sitting in some tension with the surrounding, more unconditional-sounding promise language around it — a dispute about the passage’s compositional history, not about identification, historicity, translation, or narrated causation, and a genuinely new shape of uncertainty for this project’s claim model. `mccarter-1984-ii-samuel` currently extends only through 2 Samuel 6 and has not been extended to 2 Samuel 7 this session (no WebSearch/WebFetch tool was available, and a direct check confirmed the outbound egress policy also blocks curl to esv.org/biblehub.com/biblegateway.com/biblia.com with proxy-level 403s) — per the brief’s own instruction, this stays a disclosed but unattributed note rather than a forced `scholarlyViews` entry; flagged as a carried-forward researcher item, the first place to check being `mccarter-1984-ii-samuel`’s extension to this chapter. This project renders the received text as a single narrated event throughout, matching its standing practice everywhere else (narrate the text as it stands, not a reconstructed compositional history) — no visual "layering" or split-event staging of this reading exists anywhere in this scene.',
  },
  {
