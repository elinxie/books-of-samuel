 ...
 * asset-terrain-negev.
 *
 * TEMPORARY HOME: this spec belongs in `src/scenes/ziklag/terrain.ts`; it sits
 * here only until the ADR-005 consumer migration moves it (docs/next-run.md).
 */
...
/** @deprecated Ziklag-bound global; consumers migrate to the active scene's `Terrain` (ADR-005). */
export function terrainHeight(x: number, z: number): number {
  return ZIKLAG_TERRAIN.heightAt(x, z);
}

/** @deprecated Ziklag-bound global; consumers migrate to the active scene's `Terrain` (ADR-005). */
export function buildTerrainGeometry(size?: number, segments?: number): THREE.BufferGeometry {
  return ZIKLAG_TERRAIN.buildGeometry(size, segments);
}
