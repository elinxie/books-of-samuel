...
export type TerrainFeature =
  ...
  /**
   * Radial depression with a flat-ish floor and smoothly rising sides —
   * a rock-cut pool/cistern basin, distinct from `mound`'s gaussian rise
   * (this is a gaussian *fall*, full `depth` at center tapering to 0 at
   * `radius`, the same profile `channel` uses across its width but applied
   * radially instead of along a path). See gibeon-pool's `claim-gibeon-terrain-form`.
   */
  | { kind: 'basin'; center: [number, number]; radius: number; depth: number };
...
export function createTerrain(spec: TerrainSpec): Terrain {
  ...
  for (const f of features) {
    ...
    if (f.kind === 'basin' && f.radius <= 0) {
      throw new Error('terrain basin needs a positive radius');
    }
  }
  ...
  function heightAt(x: number, z: number): number {
    ...
    for (const f of basins) {
      ...
      h -= f.depth * (1 - smoothstep01(d / f.radius));
    }
    return h;
  }
