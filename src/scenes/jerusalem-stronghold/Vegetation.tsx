...
import { STRONGHOLD_CENTER } from './layout';

/**
 * Instanced ground cover for Jerusalem — the same Judean-highland palette
 * hebron-anointing established (olive groves, scrub, surface rock), reused
 * here per the brief's "Visual composition" ("the region is not" new).
 * Scene-local component per ADR-006; placement algorithm mirrors
 * hebron-anointing/hebron-covenant's Vegetation.tsx, recentered on the ridge.
 */

const dummy = new THREE.Object3D();
const tmpColor = new THREE.Color();

function slopeDensity(d: number): number {
  if (d < 90) return 0.8;
  if (d < 200) return 0.8 - ((d - 90) / 110) * 0.42;
  return Math.max(0.18, 0.38 - (d - 200) / 700);
}
  ...
  const placements = useMemo(() => {
    ...
    const scatter = (count: number, rMax: number) => {
      ...
      while (out.length < count && guard++ < count * 40) {
        ...
        const z = STRONGHOLD_CENTER[1] + (rng() * 2 - 1) * (rMax * 0.9);
        const d = Math.hypot(x - STRONGHOLD_CENTER[0], z - STRONGHOLD_CENTER[1]);
        if (rng() > slopeDensity(d)) continue;
        out.push([x, z]);
      }
      return out;
    };
    ...
    return {
      shrubs: scatter(2600, 460),
      rocks: scatterRocks(650, 460),
      trees: scatter(230, 420),
    };
  }, []);

  useEffect(() => {
    const rng = mulberry32(240903);
    const shrubs = shrubRef.current;
    if (shrubs) {
      for (let i = 0; i < shrubCount; i++) {
        const [x, z] = placements.shrubs[i % placements.shrubs.length];
        const near = Math.hypot(x - STRONGHOLD_CENTER[0], z - STRONGHOLD_CENTER[1]) < 110;
