import { useEffect, useMemo, useRef } from 'react';
import * as THREE from 'three';
import { useAppStore } from '../../state/store';
import { mulberry32 } from '../../engine/noise';
import {
  buildCrowdLimbedGeometry,
  poseJointPositions,
  REST_LIMB_POSE,
  TUNIC_PALETTE,
  type CharacterParams,
  type CrowdLimbPose,
} from '../../engine/characters';
import { ATTENDANT_SLOTS } from './layout';

/**
 * David's attendants (`claim-reckoning-cast-scale`) — "the young men" of
 * 4:12a: David commands, attendants act. A small, disclosed presence near
 * the receiving ground, deliberately never given a separate execution
 * gesture (see poses.ts's file comment on the ADR-009 no-invented-method
 * constraint) — their presence, plus the beat caption, carries the fact
 * that they carried out the command; nothing more is staged for them. A
 * genuinely static crowd, same baked-idle-pose-bucket cost precedent as
 * `TownAmbient.tsx` (reused directly from hebron-gate for this scene's own
 * ambient background).
 */

const BUCKET_COUNT = 3;

const ATTENDANT_PARAMS: CharacterParams = {
  stature: 1.72,
  build: 0.53,
  shoulders: 1.0,
  skinColor: '#8f5b3d',
  hairColor: '#1f1712',
  beard: true,
  detail: 'crowd',
  dress: { tunicColor: TUNIC_PALETTE[0], beltColor: '#3b2416', headwear: 'bare' },
};

const IDLE_POSE_BUCKETS: CrowdLimbPose[] = [
  REST_LIMB_POSE,
  {
    legSwingL: 0.05,
    legSwingR: -0.05,
    kneeBendL: 0.08,
    kneeBendR: 0.05,
    armSwingL: 0.1,
    armSwingR: -0.08,
  },
  {
    legSwingL: -0.03,
    legSwingR: 0.05,
    kneeBendL: 0.05,
    kneeBendR: 0.1,
    armSwingL: 0.16,
    armSwingR: 0.18,
  },
];

interface AttendantFigure {
  x: number;
  z: number;
  yaw: number;
  bucket: number;
  scale: number;
  color: THREE.Color;
}

export function buildAttendantFigures(count: number, seed = 340601): AttendantFigure[] {
  const rng = mulberry32(seed);
  const color = new THREE.Color();
  const out: AttendantFigure[] = [];
  for (let i = 0; i < count; i++) {
    const [x, z] = ATTENDANT_SLOTS[i % ATTENDANT_SLOTS.length];
    color.setRGB(1, 1, 1).offsetHSL(0, 0, (rng() - 0.5) * 0.14);
    out.push({
      x,
      z,
      yaw: rng() * Math.PI * 2,
      bucket: Math.floor(rng() * BUCKET_COUNT),
      scale: 0.94 + rng() * 0.12,
      color: color.clone(),
    });
  }
  return out;
}

export function Attendants({ count, shadows }: { count: number; shadows: boolean }) {
  const terrain = useAppStore((s) => s.terrain);
  const bucketRefs = useRef<(THREE.InstancedMesh | null)[]>(Array(BUCKET_COUNT).fill(null));
  const bucketGeometries = useMemo(
    () =>
      IDLE_POSE_BUCKETS.map((pose) =>
        buildCrowdLimbedGeometry(
          ATTENDANT_PARAMS,
          poseJointPositions(ATTENDANT_PARAMS.stature, pose),
        ),
      ),
    [],
  );
  const figures = useMemo(() => buildAttendantFigures(count), [count]);
  const bucketed = useMemo(() => {
    const groups: AttendantFigure[][] = Array.from({ length: BUCKET_COUNT }, () => []);
    for (const fig of figures) groups[fig.bucket].push(fig);
    return groups;
  }, [figures]);

  useEffect(() => {
    const dummy = new THREE.Object3D();
    for (let b = 0; b < BUCKET_COUNT; b++) {
      const mesh = bucketRefs.current[b];
      if (!mesh) continue;
      const group = bucketed[b];
      for (let i = 0; i < group.length; i++) {
        const fig = group[i];
        const y = terrain.heightAt(fig.x, fig.z);
        dummy.position.set(fig.x, y, fig.z);
        dummy.rotation.set(0, fig.yaw, 0);
        dummy.scale.setScalar(fig.scale);
        dummy.updateMatrix();
        mesh.setMatrixAt(i, dummy.matrix);
        mesh.setColorAt(i, fig.color);
      }
      mesh.count = group.length;
      mesh.instanceMatrix.needsUpdate = true;
      if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
    }
  }, [bucketed, terrain]);

  return (
    <group>
      {bucketGeometries.map((geo, b) => (
        <instancedMesh
          key={b}
          ref={(el) => {
            bucketRefs.current[b] = el;
          }}
          args={[geo, undefined, Math.max(1, bucketed[b]?.length ?? 1)]}
          frustumCulled={false}
          castShadow={shadows}
        >
          <meshStandardMaterial vertexColors roughness={1} />
        </instancedMesh>
      ))}
    </group>
  );
}
