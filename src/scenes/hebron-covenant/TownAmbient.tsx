...
/**
 * Ambient Hebron townsfolk (claim-covenant-cast-scale): a working town on
 * an ordinary day, deliberately far below hebron-anointing's 150-200-figure
 * civic assembly — this is a closed political meal, not a public event, and
 * the scale contrast is itself part of what the scene says (brief's "Scale
 * assumptions"). Scattered near the house cluster, not the feast ground.
 * Two static pose buckets, matrices written once, never `useFrame` — the
 * cheapest crowd in the scene.
 */

const BUCKET_COUNT = 2;
const CHILD_FRACTION = 0.22;
...
export function buildAmbientFigures(count: number, seed = 231001): AmbientFigure[] {
