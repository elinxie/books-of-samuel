...
/**
 * David's escort/household presence at the feast (claim-covenant-cast-scale):
 * a disclosed design-choice headcount (~15-25 at high tier), drawn
 * conceptually from the same following tracked since Ziklag
 * (`davids-band`), not a new named group. Already at Hebron throughout —
 * unlike Abner's party, these figures never travel in this scene. Two
 * static pose buckets (standing rest, a weight-shifted stance), matrices
 * written once in a setup effect, never `useFrame` — this scene's cheapest
 * crowd, deliberately so (brief's performance target).
 */

const BUCKET_COUNT = 2;

const DAVID_ESCORT_PARAMS: CharacterParams = {
  stature: 1.69,
  build: 0.51,
  shoulders: 1,
  skinColor: '#8f5b3d',
...
export function buildEscortFigures(count: number, seed = 230901): EscortFigure[] {
  const rng = mulberry32(seed);
  const color = new THREE.Color();
  const out: EscortFigure[] = [];
  for (let i = 0; i < count; i++) {
    const [x, z] = DAVID_ESCORT_SLOTS[i % DAVID_ESCORT_SLOTS.length];
    color.setRGB(1, 1, 1).offsetHSL(0, 0, (rng() - 0.5) * 0.15);
    out.push({
      x,
      z,
      yaw: rng() * Math.PI * 2,
  ...
  const bucketGeometries = useMemo(
    () =>
      ESCORT_POSE_BUCKETS.map((pose) =>
        buildCrowdLimbedGeometry(
          DAVID_ESCORT_PARAMS,
          poseJointPositions(DAVID_ESCORT_PARAMS.stature, pose),
        ),
      ),
    [],
  );
