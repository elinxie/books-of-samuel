...
/**
 * David's escort at the feast (claim-covenant-cast-scale) — a disclosed
 * design count (~15-25 at high quality tier), present throughout the scene
 * rather than traveling; no narrated headcount exists for who attended, so
 * this stays static (mostly idle pose buckets, per the brief's performance
 * target "cheapest M5 scene by design") — matches hebron-anointing's
 * `JudahAssembly.tsx` cost pattern: all matrices/colors written once in a
 * setup effect, never re-posed per frame.
 */

const BUCKET_COUNT = 2;

const ESCORT_PARAMS: CharacterParams = {
  stature: 1.68,
  build: 0.5,
  shoulders: 1,
  skinColor: '#8f5b3d',
...
export function buildEscortFigures(count: number, seed = 230701): EscortFigure[] {
  ...
  const bucketGeometries = useMemo(
    () =>
      ESCORT_POSE_BUCKETS.map((pose) =>
        buildCrowdLimbedGeometry(ESCORT_PARAMS, poseJointPositions(ESCORT_PARAMS.stature, pose)),
      ),
    [],
  );
