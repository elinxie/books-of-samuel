...
GitHub Actions (`.github/workflows/deploy.yml`) configures Pages, builds, and publishes
`dist/` to GitHub Pages on every push to `main`. The repository Pages source must
be **GitHub Actions**; if it is accidentally set to deploy from a branch/root,
GitHub Pages will serve the Vite development `index.html` from the repository
instead of the built artifact.
...
Actions** if needed and rerun the **Deploy to GitHub Pages** workflow; the
workflow now runs `actions/configure-pages` before uploading the `dist/` artifact,
and the deployed `index.html` should reference `/books-of-samuel/assets/...`
files, never `/src/main.tsx`.
