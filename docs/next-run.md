...
## State right now (2026-07-24, gibeon-pool built — all three M4 scenes now built)

**`gibeon-pool` (2 Sam 2:8–32) is built** (`threejs-engineer`, this session), completing the M4 3-scene set per `docs/design/gibeon-pool-brief.md` — still **provisional**, same Fable-unavailable fallback as the rest of queue #18. `status: 'in-progress'`, `depictsDeath: true`, 13 beats (`b-context` through `b-close`), 5 viewpoints (`vp-pool` default, `vp-champions`, `vp-battle-spread`, `vp-pursuit`, `vp-ammah-hill`), 194s duration.

New scene folder `src/scenes/gibeon-pool/` (terrain/layout/poses/kitMeshes/entities + `GroundWorks`/`Vegetation`/`ChampionsGround`/`Contingents`/`PrincipalFigures`/`GibeonPoolScene`), registered in `ObservePage.tsx`. New `engine/terrain.ts` `basin` `TerrainSpec` feature kind (radial depression, flat-ish floor, smooth rising sides — the pool of Gibeon's landform), with its own engine-level test. The champions' contest renders literally 1:1 (24 figures, 12 paired grapple/fall cycles sharing one `championPose` function parameterized by pair offset); the two wider contingents are a disclosed design count (`claim-gibeon-battle-scale`, ~40/72 Abner's side + ~35/72 Joab's side at the shared `figureCount` ratio convention, landing ~99 combat figures at high tier including champions, below `gilboa-battle`'s 120–140); the hilltop rally band (≤15) is drawn from Abner's contingent via the same `ContingentFigureConfig`/`contingentFigurePose`, not an additive roster. Asahel's death (the project's first named-character-kills-named-character killing) follows the brief's precedent closely: `abnerPose`'s `spearReversed`/`strike` gestures are zero in reduced mode entirely (not just shortened), Asahel's `fallen` value snaps near-instantly in reduced mode via the same `fallDuration` helper every prior scene uses, and Joab/Abishai's `pursuerPose` implements the "stood still" reaction beat (2:23b) as a literal hold in their movement function (arrive at the death spot, don't move again until `T_PURSUIT_CONTINUES`) rather than a separate flag or camera replay.

Ten new claims per the brief's exact list (`claim-gibeon-pool-feature`, `claim-gibeon-terrain-form`, `claim-mahanaim-installation`, `claim-champions-contest`, `claim-gibeon-battle`, `claim-asahel-pursuit-death`, `claim-ammah-standoff`, `claim-gibeon-casualties`, `claim-asahel-burial-hebron-march`, `claim-gibeon-battle-scale`). **Reconciled, not duplicated**: the pre-existing `gibeon-pool` scene stub and the five pre-existing character stubs (`abner`/`ish-bosheth`/`joab`/`abishai`/`asahel`) had used different placeholder claim ids (`claim-ish-bosheth-installed`/`claim-gibeon-contest`/`claim-asahel-death`/`claim-abner-pursuit-halted`/`claim-gibeon-pool-form`) written before this brief existed in its current detailed form — those five are now **removed** from `claims.ts` and every reference to them (scene `claimIds`, character `claimIds`) repointed to the new ids; no duplicate claims exist under both naming schemes. `gibeon` `LocationEntry` bumped to `in-progress` (claimIds now cite `claim-gibeon-pool-feature`/`claim-gibeon-terrain-form`); `mahanaim` untouched (`sceneIds: []`, still `planned`, per the brief's explicit "referenced, never built" instruction). Four new asset records (`asset-terrain-gibeon-plateau`, `asset-gibeon-pool-basin`, `asset-water-plane`, `asset-gibeon-battle-kit`) plus `docs/asset-roadmap.md` and a new `docs/uncertainty-register.md` row (#15, the pool-dating question) documenting them. ESV excerpt budget: 3 quotes in this scene's own beat captions (2:20 "Is it you, Asahel?", 2:22's dramatic-irony line, 2:26's opening of Abner's plea), 187 chars total, well under the 500-char/passage-independent budget; `2sam-2`'s own `keyExcerpts` field stays untouched (neither this scene nor `hebron-anointing` populated it — both put their ESV quotes in beat captions instead, which the integrity test checks independently).

Gate green, independently re-verified by the orchestrating session (not just the build agent's own report): format:check, lint, typecheck, **349 vitest** (up from 315), build, **8/8 e2e** — plus a manual headless console-error check of `/observe/gibeon-pool` in both violence modes (0 errors either way), since this is the most combat-figure-dense scene built this session and the one most likely to surface a runtime issue the automated smoke test wouldn't catch.

**Judgment calls made without a Fable read (flag for the eventual #18 pass, same as every other M4 creative call)**: the exact bank/spread/pursuit-route/hill-of-Ammah coordinate layout (a "journey" geography the brief specified only in relative terms); the champion-pair grapple choreography's specific gesture/timing; the day-to-sundown lighting-key schedule (brief only fixed the `b-pursuit-continues` sundown anchor, 2:24, as non-placeholder — every other beat's hour stays a disclosed placeholder); the 0.556/0.486 figureCount ratios chosen to land Abner's/Joab's contingents at the brief's stated midpoints; reusing generic spear/sword kit geometry (no new "kit" claim, per the brief) rather than inventing a differentiated Gibeon-specific weapon set.
...
- `claim-gibeon-pool-feature`: whether the excavated Gibeon pool/water-tunnel predates or postdates the early Iron IIA setting of 2 Sam 2 — logged as uncertainty-register #15.
...
- `ziklag-lament`'s three ESV quotes (1:23, 1:26, 1:27) and `gibeon-pool`'s three ESV quotes (2:20, 2:22, 2:26) were both entered from model/brief recollection, not checked against a live ESV source — this sandbox has no outbound access to Bible-text sites (confirmed: proxy 403s on biblegateway/esv.org). Both read as correct ESV phrasing on cross-check against the implementing session's own knowledge, but neither is a live-source-verified check.
