...
## State right now (2026-07-26, all three M4 scenes built, Fable blocked again, branch `claude/focused-mccarthy-n6e6wt`, PR #45 draft)

**`gibeon-pool` (2 Sam 2:8–32) is built — all three M4 scenes are now built.** Same fallback pattern as the two prior scenes (`docs/model-handoff.md`): scene builds are ordinary Sonnet-tier work and don't need Fable. Per `docs/design/gibeon-pool-brief.md`: 14 beats, 5 viewpoints, pool basin terrain (negative-height `mound` + torus rim + unlit water plane — no new `TerrainSpec` kind), 24 champions rendered literally 1:1, ~99 combat figures at high tier (Abner ~40, Joab ~35), Asahel-death precedent (documentary distance, reversed-spear-grip gesture, "stood still" reaction beat per 2:23b, no wound geometry in either violence mode — the project's first named-character-kills-named-character rendering, and now the stated template for future individual killings). `status: 'in-progress'`, `depictsDeath: true`. New claims `claim-gibeon-battle-scale`, `claim-asahel-burial-hebron-march`; 4 new placeholder assets (`asset-terrain-gibeon-highlands`, `asset-gibeon-pool-basin`, `asset-water-plane`, `asset-gibeon-weapon-kit` — already in `assets.ts`/`asset-roadmap.md`, verified present, not duplicated). `gibeon` location → `in-progress`. `docs/uncertainty-register.md` row #15 (pool-dating question) added by the build. A same-session follow-up fixed a real gap the performance review found: Abner/Joab crowd contingents were rendering body-only (no weapon) for ~75 of the 99 combat figures — fixed by adding spear `InstancedMesh` to both, matching the Gilboa/ChampionsContest precedent.

**Fable hit its monthly spend limit again, mid-session, on the very first call.** A batched Fable pass was attempted for `docs/fable-review-queue.md` #18 (the full M4 scope+briefs+builds sign-off, now that all three scenes are built) — it failed immediately with "You've hit your monthly spend limit," the same failure mode as the session that wrote the three M4 briefs. Per `docs/model-handoff.md`'s fallback policy, this was **not** attempted as a Sonnet stand-in this time — a milestone sign-off review is explicitly Fable-tier work, not routine execution, so queue #18 stays open exactly as it was (still reads "Needs a real Fable pass to confirm or revise"), M4 stays `planned` at the milestone level (per `src/data/milestones.ts` — individual scenes are `in-progress`, the milestone itself only flips at sign-off, same pattern as M1–M3), and nothing was flipped to `released`.

**This is the top priority for whoever picks this up next: get a real Fable session with working budget and run the M4 sign-off (queue #18) — not more Sonnet build work.** All three scenes, all briefs, and the scope decision are ready and waiting; there is no more Sonnet-tier scene-building left on the M4 critical path. If Fable budget still isn't available, see "if Fable is still blocked" below for what's genuinely useful Sonnet-tier work in the meantime.

**A researcher pass advanced two of the four open citation gaps and closed the other two** (same session, after the failed Fable attempt — this is legitimate non-Fable work):

- `claim-gibeon-pool-form` — advanced, not closed: Tamburrini 2021 (a preprint reanalyzing Pritchard's own excavation records) dates the pool's cutting to the 10th century BCE; no source found in this pass argues for a later (post-narrative) construction. Confidence stays `low` (preprint, and "10th century BCE" alone doesn't resolve early-vs-late-century). Full framing: `docs/uncertainty-register.md` row #15.
- `claim-hebron-town-form` — advanced, not closed: now carries two competing `scholarlyViews` on Tell Rumeida's "cyclopean wall" — Ofer 1994 (reused Middle Bronze wall standing through Iron I) vs. a 2021 ZDPV reassessment (redated entirely to Iron II, centuries after David; author attribution deliberately left `TO VERIFY`/unnamed in user-facing text per its own hedge). Stays `design-placeholder`. Full framing: `docs/uncertainty-register.md` row #16.
- `claim-amalekite-messenger-account` — substantially closed: McCarter 1984 (Anchor Bible) and Reis 2006 (JSOT 30.3) now give `view-amalekite-fabrication` named, checkable proponents.
- The ziklag-lament ESV-quote live-check found and fixed one real error: 2 Sam 1:26's beat caption had a word-order mistake ("you have been very pleasant to me" instead of the actual ESV "very pleasant have you been to me") — fixed in `src/data/scenes.ts`. The other two quotes (1:23, 1:27) checked out correct.

5 new source cards added this pass; `sources/source-index.json` regenerated.

Gate green across all four of this session's commits (independently re-verified, not just taken from agent reports): 365 vitest, format:check, lint, typecheck, build, 8/8 e2e (sandbox needs `PLAYWRIGHT_CHROMIUM_PATH=/opt/pw-browsers/chromium-1194/chrome-linux/chrome`, and `npm install` first since `node_modules` wasn't present at session start).

**What's next, in priority order:**

1. **(Top) A real Fable session for queue #18's M4 sign-off** — the full `docs/fable-review-checklist.md` pass over all three scenes + briefs + scope decision, same shape as the M1/M2/M3 sign-offs. This needs actual Fable budget; do not attempt it as a Sonnet stand-in again (a milestone sign-off review is a judgment call the fallback policy doesn't cover the way routine execution is covered). Batch it — review everything in one pass rather than a partial session.
2. **If Fable is still blocked when this is picked up**, genuinely useful Sonnet-tier work that doesn't need Fable, in rough order:
   - The atlas/map UI overlay for M4's 4th goal (divided-kingdom context view) — not started; note this itself needs the atlas-vs-scene call confirmed by the same Fable pass in item 1, so building it now would itself be provisional on top of provisional. Low priority until #18 clears, or treat as another explicitly-provisional build if there's appetite.
   - Keep chasing the `claim-gibeon-pool-form`/`claim-hebron-town-form` gaps further if new sources surface (both are advanced, not closed — see above).
   - The two still-open, unrelated-to-M4 items carried forward from M3 (see immediately below) — both are genuinely sandbox-blocked, not forgotten.
3. **(Still open, unrelated to M4, carried forward, sandbox-blocked)** Real-hardware performance check of `gilboa-battle` at high tier, and the Pages-live check (`https://elinxie.github.io/books-of-samuel/`) — see Environment notes below, unchanged since first raised.
