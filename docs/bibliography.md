...
| `mccarter-1984-ii-samuel`        | II Samuel (Anchor Bible, vol. 9)                                      | reference            |
| `mccarter-1980-apology-of-david` | The Apology of David (JBL 99/4)                                       | reference            |

Note (2026-08-08): this table was already out of sync with `sources/source-cards/`
before this row was added — several cards from prior M4/M5 researcher passes
(e.g. `tamburrini-2021-pool-of-gibeon-3d`, `ussishkin-2021-cyclopean-wall-hebron`,
`ofer-1993-hebron-neaehl`, `eisenberg-ben-shlomo-2017-tel-hevron`,
`mazar-panitz-cohen-2009-beth-shean-vol3`, `mazar-2012-bar-saul-impaled`,
`master-2021-philistines-highlands`, `yasur-landau-2012-feathered-helmets`,
`stager-mountjoy-2007-ashkelon-krater`, `coughenour-1989-mahanaim`,
`finkelstein-ornan-2024-mahanaim`) are missing from this table but present and
correctly indexed in `sources/source-index.json`. Not fixed in this pass (out of
scope for the M5 citation-gap task); `sources/source-index.json` remains the
enforced, authoritative list per `src/data/sourceCards.test.ts` — this markdown
table is a convenience mirror only and a full reconciliation pass is still owed.
